...
# def translate_from_english(text, user_lang):
#     if user_lang not in INDIAN_LANGUAGES or not text.strip():
#         return text
#     try:
#         url = f"https://api.mymemory.translated.net/get?q={text}&langpair=en|{user_lang}"
#         response = requests.get(url, timeout=10)
#         data = response.json()
#         translated = data.get("responseData", {}).get("translatedText")
#         return translated if translated else text
...
def webhook():
    ...
    if intent_name == "get_disease_overview":
        response_text = "📖 DISEASE OVERVIEW\n\n"
        if not disease_param and memory.get("last_disease"):
            disease_param = memory["last_disease"]
        slug = get_slug(disease_param)
        if slug:
            url = f"https://www.who.int/news-room/fact-sheets/detail/{slug}"
            overview = fetch_overview(url)
