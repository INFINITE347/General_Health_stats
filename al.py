...
def webhook():
    ...
    # --- Symptoms ---
        ...
        if slug:
            url = f"https://www.who.int/news-room/fact-sheets/detail/{slug}"
            symptoms = fetch_symptoms(url, disease_param)
            response_text = symptoms or f"Symptoms not found for {disease_param.capitalize()}. More: {url}"
        else:
            response_text = f"Sorry, I don't have a URL for {disease_param.capitalize()}."
    ...
    # --- Treatment ---
        ...
        if slug:
            url = f"https://www.who.int/news-room/fact-sheets/detail/{slug}"
            treatment = fetch_treatment(url, disease_param)
            response_text = treatment or f"Treatment details not found for {disease_param.capitalize()}. More: {url}"
        else:
            response_text = f"Sorry, I don't have a URL for {disease_param.capitalize()}."
    ...
    # --- Prevention ---
        ...
        if slug:
            url = f"https://www.who.int/news-room/fact-sheets/detail/{slug}"
            prevention = fetch_prevention(url, disease_param)
            response_text = prevention or f"Prevention not found for {disease_param.capitalize()}. More: {url}"
        else:
            response_text = f"Sorry, I don't have a URL for {disease_param.capitalize()}."
